...
static char imageCompletionHandlerKey;
...
- (void)setImageURL:(NSURL *)imageURL withCompletionHandler:(void (^) (UIImage *image))completionHandler
{
	objc_setAssociatedObject(self, &imageCompletionHandlerKey, completionHandler, OBJC_ASSOCIATION_COPY);
    [[AsyncImageLoader sharedLoader] loadImageWithURL:imageURL target:self action:@selector(setImageWithCompletionHandler:)];
}

- (void)setImageWithCompletionHandler:(UIImage *)image
{
    [self setImage:image];
    void (^completionHandler)(UIImage *image) = objc_getAssociatedObject(self, &imageCompletionHandlerKey);
